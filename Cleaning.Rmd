<<<<<<< HEAD
---
title: "Cleaning"
author: "Marziyeh Aghaei"
date: "2024-07-22"
output:
  html_document: 
    code_folding: show
    number_sections: yes
    toc: yes
    toc_depth: 2
    toc_float: yes
---
```{r setup, include = FALSE}
knitr::opts_chunk$set(echo = TRUE)
```


```{r,message = FALSE}
# packages
require(tidyverse)
require(fixest)
require(stargazer)
require(haven)
library(usethis)
library(dplyr)
library(foreign)
library(gridExtra)
library(ggplot2)
library(openxlsx)
library(RODBC)
library(openxlsx)

# clear
rm(list = ls())

# set directories
# directory_specific <-"C:/Research/LFP"

# set general data directory
directory_data <- "C:/Users/rbm7796/Dropbox/Iranian-Women-in-the-Workforce/Data/"
# set specific data directories
directory_data_LFS <- file.path(directory_data,"/LFS")
```
```{r}
# Load Data
setwd(directory_data_LFS)

# create empty dataframes
df_84_86 <- data.frame()
df_87_99<- data.frame()

# Loop through each file, read the data, and assign to a variable
for (i in 84:99) {
    file <- paste0("LFS_", i, ".dta")
    data <- read_dta(file)
    data <- data[, order(names(data))]
    
    # if i<87 merge data with df_84_86, else if i>87 merge data with df_87_99
    if (i < 87) {
      df_84_86 <- bind_rows(df_84_86, data)
    } else {
      if (i < 96) {
        data <- subset(data, select = -IW15_Yearly)
        names(data)[names(data) == "IW10_Yearly"] <- "IW_Yearly"
      }
      df_87_99 <- bind_rows(df_87_99, data)
    }
}

# remove variable data
rm(data)

setwd(directory_data)
province_list <- read.xlsx("province_names.xlsx")
```
```{r}
#separate pkey column into 4 columns, first two digits are year, next two digits are province_code, next digit is if it is city/village, next three digit is the sample_number and the two last gigits are the family_number
df_87_99_subset <- df_87_99 %>%
  separate(pkey, into = c("year", "province_code", "city_village", "sample_number", "family_number"),
           sep = c(2, 4, 5, 8))

df_84_86_subset <- df_84_86 %>%
  separate(pkey, into = c("year", "province_code", "city_village", "sample_number", "family_number"),
           sep = c(2, 4, 5, 8))

# rename colunm F2_D04 to gender
df_87_99_subset <- df_87_99_subset %>%
  rename(
    gender = F2_D04,
    age = F2_D07,
    citizenship = F2_D08
  )

# if at least one of F3_D01 to F3_D06 is 1, then employed=1, else employed=0
df_87_99_subset <- df_87_99_subset %>%
  mutate(employed = if_else(F3_D01 == 1 | F3_D02 == 1 | F3_D03 == 1 | F3_D04 == 1 | F3_D05 == 1 | F3_D06 == 1, 1, 0))

province_list <- province_list %>%
  rename(
    province_code = Code,
    province_name = Name,
  )

df_87_99_subset <- df_87_99_subset %>%
  left_join(province_list, by = "province_code")

# keep only rows with value 2 in column gender
df_87_99_subset <- df_87_99_subset %>%
  filter(citizenship==1) %>% filter(age > 16 & age < 64)
```

```{r}
# LFP by province, urban
df_99_subset_urban <- df_87_99_subset %>%
  filter(city_village==1,year==99)

# by province_code and gender, take average on employed
df_99_LFP_by_province_urban <- df_99_subset_urban %>%
  group_by(year,province_name, gender) %>%
  summarize(avg_employed = mean(employed))

df_99_LFP_by_province_urban_wide <- df_99_LFP_by_province_urban %>%
  pivot_wider(names_from = gender, values_from = avg_employed, names_prefix = "avg_employed_gender_")

print(df_99_LFP_by_province_urban_wide)
```
```{r}
# LFP by province, rural
df_99_subset_rural <- df_87_99_subset %>%
  filter(city_village==2,year==99)

# by province_code and gender, take average on employed
df_99_LFP_by_province_rural <- df_99_subset_rural %>%
  group_by(year,province_name, gender) %>%
  summarize(avg_employed = mean(employed))

df_99_LFP_by_province_rural_wide <- df_99_LFP_by_province_rural %>%
  pivot_wider(names_from = gender, values_from = avg_employed, names_prefix = "avg_employed_gender_")

print(df_99_LFP_by_province_rural_wide)
```
```{r}
# LFP by province, urban
df_87_99_subset_urban <- df_87_99_subset %>%
  filter(city_village==1,gender==2)

# by province_code and gender, take average on employed
df_87_99_LFP_by_province_urban <- df_87_99_subset_urban %>%
  group_by(year,province_code,province_name) %>%
  summarize(avg_employed = mean(employed))

# Loop through each unique province_code and create a line plot
for (i in unique(df_87_99_LFP_by_province_urban$province_code)) {
  # Filter the data for the current province_code
  df_filtered <- df_87_99_LFP_by_province_urban %>%
    filter(province_code == i)
  
  # Create the line plot
  plot <- ggplot(df_filtered, aes(x = year, y = avg_employed, group = 1)) +
    geom_line() +
    ggtitle(paste("Province Name:", unique(df_filtered$province_name))) +
    theme_minimal()
  
  # Print the plot
  print(plot)
}
```


>>>>>>> 9de29109d949c2017a28c35f5e1f6c226a76869d
